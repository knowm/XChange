...
public class BitmaxTradeService extends BitmaxTradeServiceRaw implements TradeService {
    ...
    public String placeLimitOrder(LimitOrder limitOrder) throws IOException {
        return placeBitmaxOrder(BitmaxAdapters.adaptLimitOrderToBitmaxPlaceOrderRequestPayload(limitOrder)).getInfo().getOrderId();
    }
    ...
    public OpenOrders getOpenOrders(OpenOrdersParams params) throws IOException {
        if(params instanceof OpenOrdersParamCurrencyPair){
            return BitmaxAdapters.adaptOpenOrders(getBitmaxOpenOrders(((OpenOrdersParamCurrencyPair) params).getCurrencyPair().toString()));
        }else if(params instanceof OpenOrdersParamInstrument){
            return BitmaxAdapters.adaptOpenOrders(getBitmaxOpenOrders(((OpenOrdersParamInstrument) params).getInstrument().toString()));
        }
        else{
            throw new BitmaxException("Params must be instanceOf OpenOrdersParamCurrencyPair or OpenOrdersParamInstrument in order to get openOrders from Bitmax.");
        }
    }
    ...
    public UserTrades getTradeHistory(TradeHistoryParams params) throws IOException {
        if(params instanceof TradeHistoryParamCurrencyPair){
            return BitmaxAdapters.adaptUserTrades(getBitmaxUserTrades(((TradeHistoryParamCurrencyPair) params).getCurrencyPair().toString()));
        }else{
            throw new BitmaxException("CurrencyPair must specified in order to get usertrades from Bitmax.");
        }
    }
