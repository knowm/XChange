...
import static com.xeiam.xchange.coinsetter.CoinsetterExchange.DEPTH_FORMAT_LIST;
...
public class CoinsetterMarketDataServiceRaw extends BaseExchangeService {
   ...
   *
   * @param lookback retrieve the last <code>n</code> trades from newest to
   *          oldest (maximum supported value is 500).
   * @return the last available trades data.
   * @throws IOException indicates I/O exception.
   * @see <a href="https://www.coinsetter.com/api/marketdata/last">Market Data:
   *      Last</a>
   */
   ...
   * @throws IOException indicates I/O exception.
   * @see <a href="https://www.coinsetter.com/api/marketdata/ticker">Market
   *      Data: Ticker</a>
   */
  ...
  /**
   * Retrieves the latest Level 2 market data for the top n price levels of the
   * book.
   *
   * @param depth number of levels or pass "MAX" (Optional). Market Depth to
   *          request.
   * @param exchange "SMART" (Default) or "COINSETTER" for Coinsetter-only data
   *          (Optional). Filter for given exchange or SMART for aggregated
   *          data.
   * @return the latest Level 2 market data for the top n price levels of the
   *         book.
   * @throws IOException indicates I/O exception.
   * @see #getCoinsetterFullDepth(String)
   * @see <a href="https://www.coinsetter.com/api/marketdata/depth">Market Data:
   *      Depth</a>
   */
   ...
   * <p>
   * New full-depth snapshot is generated every 60 seconds. So, there is no need
   * to call this more than once a minute.
   * </p>
   *
   * @param exchange "SMART" (Default) or "COINSETTER" for Coinsetter-only data
   *          (Optional). Filter for given exchange or deafult to SMART for
   *          aggregated data.
   * @return the whole aggreated order book.
   * @throws IOException indicates I/O exception.
   * @see <a href="https://www.coinsetter.com/api/marketdata/full_depth">Market
   *      Data: Full-Depth</a>
   */
  ...
  /**
   * Ever want to know at a glance how much it will cost to buy 10 BTC or 30BTC?
   * Use this call to do so. It returns an indicative or synthetic quote for a
   * given quantity based on the current state of the order book. Using this
   * request you can retrieve a VWAP (volume-weighted average price) quote in
   * USD for a given BTC quantity. For example, if you want to know what you
   * will pay to buy 10 BTC, you make a GET request to our api and provide 10 as
   * the quantity, the value returned will be the VWAP in dollars (ex. $572.68,
   * total $5726.80).
   *
   * @param quantity if a positive quantity is passed in, it will query the BID
   *          side to determine the volume-weighted average price and total
   *          order cost. For negative quantity, the ASK side. (Must be less
   *          than 150 and divisible by 5).
   * @param symbol trade symbol to find amount in (Currently only BTCUSD is
   *          supported and is the default).
   * @return the quote.
   * @throws IOException indicates I/O exception.
   * @see <a href="https://www.coinsetter.com/api/marketdata/quote">Market Data:
   *      Quote</a>
   */
