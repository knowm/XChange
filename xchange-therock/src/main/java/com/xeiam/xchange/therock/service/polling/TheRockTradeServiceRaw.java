...
public class TheRockTradeServiceRaw extends TheRockBasePollingService {
  ...
  public TheRockOrder placeOrder(CurrencyPair currencyPair, BigDecimal amount, BigDecimal price, TheRockOrder.Side side, TheRockOrder.Type type)
    ...
    return theRockAuthenticated.placeOrder(
        new TheRock.Pair(currencyPair),
        exchange.getExchangeSpecification().getApiKey(),
        this.signatureCreator,
        exchange.getNonceFactory(),
        new TheRockOrder(new TheRock.Pair(currencyPair), side, type, amount, price)
    );
  }

/*
  public TheRockOrder[] getTheRockOpenOrders(CurrencyPair currencyPair) throws IOException {
    TheRockOpenOrdersResponse response = null;
    try {
      response = theRockAuthenticated.getOpenOrders(
          exchange.getExchangeSpecification().getApiKey(),
          signatureCreator,
          new TheRock.Pair(currencyPair),
          new TheRockRequest());
  ...
  public TheRockBaseResponse cancelTheRockOrder(CurrencyPair currencyPair, Long orderId) throws IOException {
    return theRockAuthenticated.deleteOrder(
        exchange.getExchangeSpecification().getApiKey(),
        signatureCreator,
        new TheRock.Pair(currencyPair),
        new TheRockCancelOrderRequest(orderId));
  }

  public TheRockUserTrade[] getTheRockUserTransactions(CurrencyPair currencyPair, Integer offset, Integer limit, Long sinceTradeId)
      throws IOException {
    TheRockTradeHistoryResponse result = theRockAuthenticated.getTradeHistory(
        exchange.getExchangeSpecification().getApiKey(),
        signatureCreator,
        new TheRock.Pair(currencyPair),
        new TheRockTradeHistoryRequest(limit, offset, sinceTradeId));
