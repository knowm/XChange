...
public class KucoinMarketDataServiceRaw extends KucoinBaseService {
  ...
  public KucoinResponse<KucoinTicker> getKucoinTicker(CurrencyPair currencyPair)
      throws IOException {
    try {
  ...
  public KucoinResponse<KucoinOrderBook> getKucoinOrderBook(
    ...
    try {
      return checkSuccess(
          kucoin.orders(KucoinAdapters.adaptCurrencyPair(currencyPair), null, limit));
    } catch (KucoinException e) {
      throw new ExchangeException(e.getMessage());
    }
  }

  public KucoinResponse<List<KucoinDealOrder>> getKucoinTrades(
      CurrencyPair currencyPair, Integer limit, Long since) throws IOException {
    try {
      return checkSuccess(
          kucoin.dealOrders(KucoinAdapters.adaptCurrencyPair(currencyPair), limit, since));
    } catch (KucoinException e) {
      throw new ExchangeException(e.getMessage());
    }
  }
