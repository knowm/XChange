...
public class KucoinAccountService extends KucoinAccountServiceRaw implements AccountService {
  ...
  public AccountInfo getAccountInfo() throws IOException {
    try {
      ...
      Map<String, List<Balance>> balancesByAccountType =
          accounts.stream()
              .collect(
      ...
      balancesByAccountType.forEach(
          (accountType, balances) -> {
            Wallet wallet = Wallet.Builder.from(balances).id(accountType).name(accountType).build();
            wallets.add(wallet);
          });
  ...
  public String withdrawFunds(WithdrawFundsParams params) throws IOException {
    ...
    ApplyWithdrawApiRequest request =
  ...
  public DepositAddressResponse createDepositAddress(Currency currency, String chain)
      throws IOException {
    try {

      CreateDepositAddressApiRequest request =
          CreateDepositAddressApiRequest.builder()
