...
public class CoinfloorAdapters {
	...
	public Map<String, Object> adaptBalances(String data) throws ExchangeException{
	    ...
	    for (CoinfloorAssetBalance assetBalancePair : funds) {
	      String currency = assetBalancePair.getAsset();
	...
	private LimitOrder adaptOrder(CoinfloorOrder order) throws ExchangeException{
		...
		String baseCurrency = order.getBase();
		String counterCurrency = order.getCounter();
		
		OrderType type = null;
		if(order.getBaseQty().doubleValue() > 0){type = OrderType.BID;}
		else if(order.getBaseQty().doubleValue() <= 0){type = OrderType.ASK;}
	...
	public Map<String, Object> adaptTicker(String data){
		...
		Ticker genericTicker = new Ticker.TickerBuilder().withVolume(rawRetObj.getVolume()).withAsk(ask).
				withCurrencyPair(new CurrencyPair("BTC","GBP")).
				withBid(bid).withHigh(high).withLow(low).withLast(last).build();
	...
	public Map<String, Object> adaptTickerUpdate(String data){
		...
		Ticker genericTicker = new Ticker.TickerBuilder().withCurrencyPair(new CurrencyPair(rawRetObj.getBase(), rawRetObj.getCounter())).
				withLast(last).withBid(bid).withAsk(ask).withLow(low).withHigh(high).withVolume(volume).build();
	...
	public Map<String, Object> adaptOrdersMatched(String data){
		...
		Trade trade = new Trade(type, rawRetObj.getBaseQty(), new CurrencyPair(rawRetObj.getBase(), rawRetObj.getCounter()), limitPrice, new Date(), String.valueOf(rawRetObj.getId()), null);
	...
	public Map<String, Object> adaptBalancesChanged(String data){
		...
		if(cachedAccountInfo == null){
			String currency = rawRetObj.getAsset();
	    ...
	    AccountInfo accountInfo = new AccountInfo(null, newWallets);
	    cachedAccountInfo = accountInfo;
	...
	public AccountInfo getCachedAccountInfo(){
		if(cachedAccountInfo == null){throw new ExchangeException("getBalances method has not been called yet, or balance data has not been recieved!");}
	...
	public OrderBook getCachedOrderBook(){
		if(cachedOrderBook == null){throw new ExchangeException("watchOrders method has not been called yet!");}
