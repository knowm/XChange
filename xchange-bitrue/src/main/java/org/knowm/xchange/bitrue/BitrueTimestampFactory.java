...
public class BitrueTimestampFactory implements SynchronizedValueFactory<Long> {

  private static final Logger LOG = LoggerFactory.getLogger(BitrueTimestampFactory.class);

  private final Bitrue binance;
  ...
  public long deltaServerTime() throws IOException {
    ...
    if (deltaServerTime == null || deltaServerTimeExpire <= System.currentTimeMillis()) {
      ...
      // Do a little warm up
      Date serverTime = new Date(bitrueTime().getServerTime().getTime());
  ...
  private BitrueTime bitrueTime() throws IOException {
