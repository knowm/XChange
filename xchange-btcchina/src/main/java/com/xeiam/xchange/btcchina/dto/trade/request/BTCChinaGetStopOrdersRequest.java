...
public class BTCChinaGetStopOrdersRequest extends BTCChinaRequest {
   ...
   * @param stopPrice Price to filter on. For bid stop orders, will return all stop orders less than or equal this stop price. For ask stop orders, will return all stop orders greater than or equal to
   *          this stop price.
  ...
  public BTCChinaGetStopOrdersRequest(String status, String type, BigDecimal stopPrice, Integer limit, Integer offset, String market) {
    ...
    this.params = String.format("[%s,%s,%s,%d,%d,%s]", status == null ? "" : status, type == null ? "" : type, stopPrice == null ? "" : stopPrice.stripTrailingZeros().toPlainString(),
        limit == null ? 1000: limit.intValue(), offset == null ? 0 : offset.intValue(), market == null ? BTCChinaExchange.DEFAULT_MARKET : market);
  }
