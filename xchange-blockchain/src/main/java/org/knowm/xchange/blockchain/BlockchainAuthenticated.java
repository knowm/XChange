...
public interface BlockchainAuthenticated extends Blockchain {
   ...
   *
   * @return This returns a map where the key {@link String} represents the primary data and the
   *     value which is an instance of type {@link BlockchainAccountInformation} is a list of
   *     account balances
   */
  @Path("/accounts")
  @GET
  ...
  /**
   * Request a withdrawal Call GET /whitelist first to retrieve the ID of the beneficiary.
   *
  ...
  BlockchainWithdrawal postWithdrawFunds(BlockchainWithdrawalParams blockchainWithdrawalRequest)
  ...
  @Path("/withdrawals")
  @GET
  List<BlockchainWithdrawal> getWithdrawFunds(
      @QueryParam("from") Long startTime, @QueryParam("to") Long endTime);
  ...
  @Path("/deposits")
  @GET
  List<BlockchainDeposits> depositHistory(
      @QueryParam("from") Long startTime, @QueryParam("to") Long endTime);

  /**
   * Get a list orders
   *
   * @return live and historic orders, defaulting to live orders. Returns at most 100 results, use
   *     timestamp to paginate for further results
   */
  @Path("/orders")
  @GET
  ...
  @Path("/orders")
  @GET
  List<BlockchainOrder> getOrdersBySymbol(@QueryParam("symbol") String symbol)
      throws IOException, BlockchainException;
   ...
   * @param endTime
   * @param limit
   * @return filled orders, including partial fills. Returns at most 100 results, use timestamp to
