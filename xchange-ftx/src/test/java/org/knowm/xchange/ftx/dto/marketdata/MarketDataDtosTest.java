...
public class MarketDataDtosTest {
    ...
    public void marketsDtoUnmarshall() throws IOException {
        ...
        InputStream is =
                TradeDtosTest.class.getResourceAsStream(
                        "/responses/example-ftxMarkets.json");

        // Use Jackson to parse it
        ObjectMapper mapper = new ObjectMapper();
        FtxResponse<FtxMarketsDto> ftxResponse = mapper.readValue(is, new TypeReference<FtxResponse<FtxMarketsDto>>() {});

        // Verify that the example data was unmarshalled correctly
        assertThat(ftxResponse.getResult().getMarketList().size()).isEqualTo(6);

        assertThat(ftxResponse.getResult().getMarketList().get(0).getAsk()).isEqualTo(BigDecimal.valueOf(128.09));
    ...
    public void currencyDeserializerTest() throws IOException{
        ...
        ftxResponse.getResult().getMarketList().forEach(ftxMarketDto -> {
            System.out.println(CurrencyPairDeserializer.getCurrencyPairFromString(ftxMarketDto.getName()).toString());
        });
