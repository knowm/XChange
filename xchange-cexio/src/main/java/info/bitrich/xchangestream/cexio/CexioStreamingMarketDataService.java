...
public class CexioStreamingMarketDataService implements StreamingMarketDataService {
    ...
    static class OrderBookUpdateConsumer implements io.reactivex.functions.Function<CexioWebSocketOrderBookSubscribeResponse, OrderBook> {
		...
		public OrderBook apply(CexioWebSocketOrderBookSubscribeResponse t) throws Exception {
			...
			if (prevID == null || prevID.add(BigInteger.ONE).compareTo(t.id) == 0) {
        		...
        		retVal = orderBookSoFar;
        		orderBookSoFar = new OrderBook(new Date(), new ArrayList<LimitOrder>(), new ArrayList<LimitOrder>());
        		prevID = null;
        	}
			return retVal;
		}
    ...
    public Observable<OrderBook> getOrderBook(CurrencyPair currencyPair, Object... args) {
        ...
        Observable<JsonNode> jsonNodeObservable = streamingOrderDataService.subscribeChannel(channelNameForPair, currencyPair);
        OrderBookUpdateConsumer orderBookConsumer = new OrderBookUpdateConsumer(streamingOrderDataService);
        return jsonNodeObservable
                .map(s -> {
                	JsonNode dataNode = s.get("data");
                	return mapper.readValue(dataNode.toString(), CexioWebSocketOrderBookSubscribeResponse.class);
                })
