...
public class KunaMarketDataService extends KunaMarketDataServiceRaw implements MarketDataService {
  ...
  public OrderBook getOrderBook(CurrencyPair currencyPair, Object... args) throws IOException {
    ...
    List<LimitOrder> asks = Arrays.stream(kunaAskBid.getAsks()).map(kunaOrder -> mapKunaOrder2LimitOrder(kunaOrder, currencyPair))
                                  .collect(Collectors.toList());
    List<LimitOrder> bids = Arrays.stream(kunaAskBid.getBids()).map(kunaOrder -> mapKunaOrder2LimitOrder(kunaOrder, currencyPair))
                                  .collect(Collectors.toList());
  ...
  protected Ticker mapKunaTicker2Ticker(KunaTimeTicker kunaTimeTicker, CurrencyPair currencyPair) {
    ...
    Ticker.Builder builder = new Ticker.Builder().currencyPair(currencyPair).timestamp(timestamp).ask(kunaTicker.getBuy()).bid(kunaTicker.getSell())
                                                 .high(kunaTicker.getHigh()).low(kunaTicker.getLow()).last(kunaTicker.getLast())
  ...
  protected LimitOrder mapKunaOrder2LimitOrder(KunaOrder kunaOrder, CurrencyPair currencyPair) {
    ...
    builder.id(String.valueOf(kunaOrder.getId())).currencyPair(currencyPair).timestamp(kunaOrder.getCreatedAt()).orderStatus(Order.OrderStatus.NEW)
           .orderType(orderType).averagePrice(kunaOrder.getAveragePrice()).limitPrice(kunaOrder.getPrice()).originalAmount(kunaOrder.getVolume())
           .remainingAmount(kunaOrder.getRemainingVolume()).cumulativeAmount(kunaOrder.getExecutedVolume());
    return builder.build();
  }
  ...
  protected Trade mapKunaTrade2Trade(KunaTrade kunaTrade, CurrencyPair currencyPair) {
    Trade.Builder builder = new Trade.Builder().currencyPair(currencyPair).id(String.valueOf(kunaTrade.getId())).price(kunaTrade.getPrice())
                                               .timestamp(kunaTrade.getCreatedAt()).originalAmount(kunaTrade.getVolume());
    return builder.build();
  }
}
