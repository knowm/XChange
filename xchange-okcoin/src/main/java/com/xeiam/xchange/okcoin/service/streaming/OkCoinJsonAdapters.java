...
public class OkCoinJsonAdapters {
  ...
  public static Ticker adaptTicker(OkCoinStreamingTicker ticker, CurrencyPair currencyPair) {
      return new Ticker.Builder().currencyPair(currencyPair).high(ticker.getHigh()).low(ticker.getLow()).bid(
              ticker.getBuy()).ask(ticker.getSell()).last(ticker.getLast()).
              volume(new BigDecimal(ticker.getVol().replaceAll(",", ""))).timestamp(new Date(ticker.getTimestamp())).build();
  }
