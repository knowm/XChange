...
public interface Abucoins {
  ...
  AbucoinsTrade[] getTradesAfter(@PathParam("product-id") String product_id, @PathParam("after") String after, @PathParam("limit") String limit)
      throws IOException;

  @GET
  @Path("products/{product-id}/trades?before={before}&limit={limit}")
  AbucoinsTrade[] getTradesBefore(@PathParam("product-id") String product_id, @PathParam("before") String before, @PathParam("limit") String limit)
      throws IOException;

  @GET
  @Path("products/{product-id}/candles?granularity={granularity}&start={start}&end={end}")
  AbucoinsHistoricRates getHistoricRates(@PathParam("product-id") String product_id, @PathParam("granularity") String granularity,
