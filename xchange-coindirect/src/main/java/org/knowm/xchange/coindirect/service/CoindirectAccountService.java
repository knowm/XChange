...
public class CoindirectAccountService extends CoindirectAccountServiceRaw implements AccountService {
    ...
    public AccountInfo getAccountInfo() throws IOException {
        ...
        for(CoindirectWallet coindirectWallet : coindirectWallets) {
            balance = new Balance(CoindirectAdapters.toCurrency(coindirectWallet.currency.code), coindirectWallet.balance);
            wallet = new Wallet(String.valueOf(coindirectWallet.id), coindirectWallet.description, Arrays.asList(balance));
            wallets.add(wallet);
        }
