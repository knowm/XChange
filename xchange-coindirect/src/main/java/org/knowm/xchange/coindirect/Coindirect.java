...
public interface Coindirect {
    ...
    CoindirectOrderbook getExchangeOrderBook(@QueryParam("symbol") String symbol) throws IOException, CoindirectException;

    @GET
    @Path("/api/v1/exchange/historical/trades/{market}/{history}")
    CoindirectTrades getHistoricalExchangeTrades(@PathParam("market") String market, @PathParam("history") String history) throws IOException, CoindirectException;

    @GET
    @Path("/api/v1/exchange/historical/{market}/{history}/{grouping}")
    CoindirectTicker getHistoricalExchangeData(@PathParam("market") String market, @PathParam("history") String history, @PathParam("grouping") String grouping) throws IOException, CoindirectException;

    @GET
    @Path("/api/v1/exchange/market")
    List<CoindirectMarket> listExchangeMarkets(@QueryParam("max") long max) throws IOException, CoindirectException;
