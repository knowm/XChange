...
public class BitgetTradeServiceRaw extends BitgetBaseService {
  ...
  public List<BitgetFillDto> bitgetFills(TradeHistoryParams params) throws IOException {
    // get arguments
    Instrument instrument = params instanceof TradeHistoryParamInstrument ? ((TradeHistoryParamInstrument) params).getInstrument() : null;
    ...
    return bitgetAuthenticated.fills(apiKey, bitgetDigest, passphrase, exchange.getNonceFactory(),
        BitgetAdapters.toString(instrument), limit, orderId, from, to, lastTradeId).getData();
  }
