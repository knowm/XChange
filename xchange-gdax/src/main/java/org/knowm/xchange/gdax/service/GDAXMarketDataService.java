...
public class GDAXMarketDataService extends GDAXMarketDataServiceRaw implements MarketDataService {
   ...
   * <p>
   * If invoked with only the currency pair, the method will
   * make a single api call, returning the default number 
   * (currently 100) of the most recent trades.
   * </p>
   * If invoked with either optional argument the other must be
   * specified as well.
   *
   * @param  currencyPair Currency pair to obtain trades for (required)
   * @param  args[0] fromTradeId (Long) Return Trades with tradeIds greater than or equal to this value. Additional values may be returned. (optional)
   * @param  args[1] toTradeId (Long) Return Trades with tradeIds up to but not including this value (optional)
   * 
...
//		  return GDAXAdapters.adaptTrades(gdaxTrades, currencyPair);		  
		  ...
		  for (;;) {	  
			  ...
			  log.debug("latestTradeId: {}, earliest-latest: {}-{}, trades: {}", 
					  latestTradeId, gdaxTrades.getEarliestTradeId(), gdaxTrades.getLatestTradeId(), gdaxTrades);
			  
			  latestTradeId = gdaxTrades.getEarliestTradeId();
			  
			  if (gdaxTradesNew.getEarliestTradeId() == null) {break;}
			  if (gdaxTrades.getEarliestTradeId() <= fromTradeId) {break;}
		  }
		  
		  log.debug("earliest-latest: {}-{}", gdaxTrades.getEarliestTradeId(), gdaxTrades.getLatestTradeId());
