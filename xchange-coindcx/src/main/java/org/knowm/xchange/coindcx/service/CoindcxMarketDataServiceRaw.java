...
public class CoindcxMarketDataServiceRaw extends CoindcxBaseService {
	...
	public CoindcxOrderBook getCoindcxOrderBook(CurrencyPair currencyPair,Object... args) throws IOException{
		return coindcx.getOrderBook(getSymbol(currencyPair,args));
	}
	
	 public List<CoindcxTrade> getCoindcxTrades(CurrencyPair currencyPair,Object... args) throws IOException{
	...
	private String getSymbol(CurrencyPair currencyPair,Object... args){
		...
		if(args[0]== null){
			...
			if(("B").equals(args[0].toString())){
				return "B-"+currencyPair.base.getCurrencyCode().toUpperCase()+"_"+currencyPair.counter.getCurrencyCode();
			}else if(("I").equals(args[0].toString())){
				return "I-"+currencyPair.base.getCurrencyCode().toUpperCase()+"_"+currencyPair.counter.getCurrencyCode();
			}else if(("HB").equals(args[0].toString())){
				return "HB-"+currencyPair.base.getCurrencyCode().toUpperCase()+"_"+currencyPair.counter.getCurrencyCode();
			}else if(("H").equals(args[0].toString())){
				return "H-"+currencyPair.base.getCurrencyCode().toUpperCase()+"_"+currencyPair.counter.getCurrencyCode();
			}else if(("BM").equals(args[0].toString())){
				return "BM-"+currencyPair.base.getCurrencyCode().toUpperCase()+"_"+currencyPair.counter.getCurrencyCode();
			}else{
				throw new ExchangeException("agrs[0] must be any of this B, I, HB, H, BM");
			}
		}
