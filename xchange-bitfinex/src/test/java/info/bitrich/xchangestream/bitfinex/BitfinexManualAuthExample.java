...
//FIXME - not getting balances as they're published immediately on connection but therefore missed
        //Thread.sleep(5000);
        try {
...
//            );
            ...
            exchange.getStreamingAccountService().getBalanceChanges(Currency.USD, "exchange").subscribe(
                t -> LOG.info("GENERIC EXCHANGE BALANCE: {}", t),
                throwable -> LOG.error("ERROR: ", throwable)
            );
