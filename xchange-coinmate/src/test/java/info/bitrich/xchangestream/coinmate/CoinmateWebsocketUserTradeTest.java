...
public class CoinmateWebsocketUserTradeTest {
    ...
    public void coinmateWebsocketOpenOrdersTest() throws IOException {
        String message = StreamingObjectMapperHelper.getObjectMapper().readTree(
                this.getClass().getResource("/user-trade.json").openStream()).toString();

        List<CoinmateWebSocketUserTrade> websocketUserTrades = StreamingObjectMapperHelper.getObjectMapper().readValue(message, new TypeReference<List<CoinmateWebSocketUserTrade>>() {});
