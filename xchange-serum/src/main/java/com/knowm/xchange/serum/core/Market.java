...
public class Market {
    ...
    public Market(final MarketStat decoded,
    ...
    public double baseSizeLotsToNumber(final BigDecimal size) {
        return divideBNToNumber(
                size.multiply(BigDecimal.valueOf(this.decoded.getBaseLotSize())),
                baseSplTokenMultiplier());
    }

    public double priceLotsToNumber(final BigDecimal price) {
        return divideBNToNumber(
                price.multiply(BigDecimal.valueOf(this.decoded.getQuoteLotSize()).multiply(baseSplTokenMultiplier())),
    ...
    public double divideBNToNumber(final BigDecimal numerator, final BigDecimal denominator) {
        ...
        return quotient.doubleValue() + rem.divide(new BigDecimal(gcd), RoundingMode.HALF_UP).doubleValue() / denominator.toBigInteger().divide(gcd).doubleValue();
    }
    ...
    public BigDecimal priceNumberToLots(final BigInteger price, final int decimalPlaces) {
        final BigDecimal a = new BigDecimal(price)
                .multiply(quoteSplTokenMultiplier())
                .multiply(new BigDecimal(decoded.getBaseLotSize()));
        final BigDecimal b = baseSplTokenMultiplier()
                .multiply(new BigDecimal(decoded.getQuoteLotSize()));
