...
public class MarketStatLayoutV1 extends MarketLayout {
    ...
    @Bin(order = 21, name = "blob7", type = BinType.BYTE_ARRAY) public byte[] blob7;

    public static final StructDecoder<MarketStat> DECODER = bytes -> {
        final MarketStatLayoutV1 deserialized = JBBPParser.prepare(""
                + "byte[5] blob5; "
                + "accountFlags {"
                + " byte[8] bytes; "
                + "} "
                ...
                + "byte[8] feeRateBps; "
                + "byte[7] blob7;"
        ).parse((bytes)).mapTo(new MarketStatLayoutV1());
