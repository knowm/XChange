...
public class MarketStatLayoutV2 extends MarketLayout {
    @Bin(order = 1, name = "blob5", type = BinType.BYTE_ARRAY) public byte[] blob5;

    @Bin(order = 2, name = "accountFlags", type = BinType.STRUCT) public AccountFlagsLayout accountFlags;

    @Bin(order = 3, name = "ownAddress", type = BinType.STRUCT) public PublicKeyLayout ownAddress;

    @Bin(order = 4, name = "vaultSignerNonce", type = BinType.BYTE_ARRAY) public byte[] vaultSignerNonce;

    @Bin(order = 5, name = "baseMint", type = BinType.STRUCT) public PublicKeyLayout baseMint;
    @Bin(order = 6, name = "quoteMint", type = BinType.STRUCT) public PublicKeyLayout quoteMint;

    @Bin(order = 7, name = "baseVault", type = BinType.STRUCT) public PublicKeyLayout baseVault;
    @Bin(order = 8, name = "baseDepositsTotal", type = BinType.BYTE_ARRAY) public byte[] baseDepositsTotal;
    @Bin(order = 9, name = "baseFeesAccrued", type = BinType.BYTE_ARRAY) public byte[] baseFeesAccrued;

    @Bin(order = 10, name = "quoteVault", type = BinType.STRUCT) public PublicKeyLayout quoteVault;
    @Bin(order = 11, name = "quoteDepositsTotal", type = BinType.BYTE_ARRAY) public byte[] quoteDepositsTotal;
    @Bin(order = 12, name = "quoteFeesAccrued", type = BinType.BYTE_ARRAY) public byte[] quoteFeesAccrued;

    @Bin(order = 13, name = "quoteDustThreshold", type = BinType.BYTE_ARRAY) public byte[] quoteDustThreshold;

    @Bin(order = 14, name = "requestQueue", type = BinType.STRUCT) public PublicKeyLayout requestQueue;
    @Bin(order = 15, name = "eventQueue", type = BinType.STRUCT) public PublicKeyLayout eventQueue;

    @Bin(order = 16, name = "bids", type = BinType.STRUCT) public PublicKeyLayout bids;
    @Bin(order = 17, name = "asks", type = BinType.STRUCT) public PublicKeyLayout asks;

    @Bin(order = 18, name = "baseLotSize", type = BinType.BYTE_ARRAY) public byte[] baseLotSize;
    @Bin(order = 19, name = "quoteLotSize", type = BinType.BYTE_ARRAY) public byte[] quoteLotSize;

    @Bin(order = 20, name = "feeRateBps", type = BinType.BYTE_ARRAY) public byte[] feeRateBps;

    @Bin(order = 21, name = "referrerRebatesAccrued", type = BinType.BYTE_ARRAY) public byte[] referrerRebatesAccrued;

    @Bin(order = 22, name = "blob7", type = BinType.BYTE_ARRAY) public byte[] blob7;

    public static final StructDecoder<MarketStat> DECODER = bytes -> {
        final MarketStatLayoutV2 deserialized = JBBPParser.prepare(""
                + "byte[5] blob5; "
                + "accountFlags {"
                + " byte[8] bytes; "
                + "} "
                ...
                + "byte[8] quoteLotSize; "
                + "byte[8] feeRateBps; "
                + "byte[8] referrerRebatesAccrued; "
                + "byte[7] blob7;"
        ).parse((bytes)).mapTo(new MarketStatLayoutV2());
        return new MarketStatV2(
                ...
                decodeLong(deserialized.referrerRebatesAccrued));
    };

    public static class MarketStatV2 implements MarketStat {
        ...
        public long referrerRebatesAccrued;

        public MarketStatV2(AccountFlagsLayout.AccountFlags accountFlags,
