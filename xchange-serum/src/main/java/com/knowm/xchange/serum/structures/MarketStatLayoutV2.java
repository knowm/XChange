...
public class MarketStatLayoutV2 extends MarketLayout {
  ...
  @Bin(order = 21, name = "referrerRebatesAccrued", type = BinType.BYTE_ARRAY)
  public byte[] referrerRebatesAccrued;

  @Bin(order = 22, name = "blob7", type = BinType.BYTE_ARRAY)
  public byte[] blob7;

  public static final StructDecoder<MarketStat> DECODER =
      bytes -> {
        final MarketStatLayoutV2 deserialized =
            JBBPParser.prepare(
                    ""
                        + "byte[5] blob5; "
                        + "accountFlags {"
