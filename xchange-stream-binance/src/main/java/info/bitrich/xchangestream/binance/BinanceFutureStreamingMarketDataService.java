...
public class BinanceFutureStreamingMarketDataService implements StreamingMarketDataService {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(BinanceFutureStreamingMarketDataService.class);
   ...
   */
  private static final Scheduler bookSnapshotsScheduler = Schedulers.from(Executors.newSingleThreadExecutor(
          new ThreadFactoryBuilder().setDaemon(true).setNameFormat("binancefuture-book-snapshots-%d").build()));
  ...
  private final Map<Instrument, Observable<DepthBinanceFutureWebSocketTransaction>> orderBookRawUpdatesSubscriptions;
  ...
  public BinanceFutureStreamingMarketDataService(BinanceStreamingService service, BinanceFutureMarketDataService marketDataService,
                                                 Runnable onApiCall, final String orderBookUpdateFrequencyParameter,
                                                 boolean realtimeOrderBookTicker, int oderBookFetchLimitParameter) {
  ...
  public Observable<BinanceBookTicker> getRawBookTicker(CurrencyPair currencyPair, Object... args) {
    ...
    return bookTickerSubscriptions.computeIfAbsent(currencyPair, s -> triggerObservableBody(rawBookTickerStream(currencyPair)).share());
  }
  ...
  public Observable<BinanceKline> getKlines(CurrencyPair currencyPair, KlineInterval interval) {
    if (!service.isLiveSubscriptionEnabled() && !service.getKlineSubscription().contains(currencyPair, interval)) {
      throw new UpFrontSubscriptionRequiredException();
    }
    return klineSubscriptions.compute(currencyPair, (c, v) -> {
  ...
  private Observable<BinanceKline> klinesStream(Instrument instrument, KlineInterval interval) {
    return service.subscribeChannel(getChannelPrefix(instrument) + "@" + KLINE.getType() + "_" + interval.code())
            .map(it -> this.<KlineBinanceWebSocketTransaction>readTransaction(it, KLINE_TYPE, "kline").getData()
                    .toBinanceKline(instrument instanceof FuturesContract))
            .filter(binanceKline -> binanceKline.getInstrument().equals(instrument) && binanceKline.getInterval().equals(interval));
  }
  ...
  public Observable<OrderBookUpdate> getOrderBookUpdates(CurrencyPair currencyPair, Object... args) {
    ...
    return orderBookUpdatesSubscriptions.computeIfAbsent(currencyPair, this::initOrderBookUpdateIfAbsent);
  }
  ...
  public Observable<Trade> getTrades(CurrencyPair currencyPair, Object... args) {
    return getRawTrades(currencyPair, args).map(
            rawTrade -> new Trade.Builder().type(BinanceAdapters.convertType(rawTrade.isBuyerMarketMaker()))
                    .originalAmount(rawTrade.getQuantity()).instrument(currencyPair).price(rawTrade.getPrice())
                    .makerOrderId(getMakerOrderId(rawTrade)).takerOrderId(getTakerOrderId(rawTrade))
                    .timestamp(new Date(rawTrade.getTimestamp())).id(String.valueOf(rawTrade.getTradeId())).build());
  }
  ...
  private Observable<OrderBookUpdate> createOrderBookUpdatesObservable(Instrument currencyPair) {
    return orderBookRawUpdatesSubscriptions.get(currencyPair)
            .flatMap(depthTransaction -> observableFromStream(extractOrderBookUpdates(currencyPair, depthTransaction))).share();
  }
  ...
  public void unsubscribe(Instrument currencyPair, BinanceSubscriptionType subscriptionType) {
    if (subscriptionType == KLINE) {
      klineSubscriptions.computeIfPresent(currencyPair, (k, intervalMap) -> {
        intervalMap.keySet().forEach(klineInterval -> unsubscribeKline(currencyPair, klineInterval));
        return null;
      });
    } else {
  ...
  private void unsubscribe(Instrument currencyPair, BinanceSubscriptionType subscriptionType, KlineInterval klineInterval) {
    ...
    if (!service.isLiveSubscriptionEnabled()) {
      throw new UnsupportedOperationException(
              "Unsubscribe not supported for Binance when live Subscription/Unsubscription is disabled. " + "Call BinanceStreamingExchange.enableLiveSubscription() to active it");
    }
  ...
  private String getChannelId(Instrument currencyPair, BinanceSubscriptionType subscriptionType, KlineInterval klineInterval) {
    return getChannelPrefix(currencyPair) + "@" + subscriptionType.getType() + (klineInterval != null ? "_" + klineInterval.code() : "");
  }
  ...
  private void initTickerSubscription(Instrument instrument) {
    if (realtimeOrderBookTicker) {
      bookTickerSubscriptions.put(instrument, triggerObservableBody(rawBookTickerStream(instrument)).share());
    } else {
      tickerSubscriptions.put(instrument, triggerObservableBody(rawTickerStream(instrument)).share());
    }
  }
  ...
  private Observable<BinanceTicker24h> rawTickerStream(Instrument instrument) {
    return service.subscribeChannel(channelFromCurrency(instrument, BinanceSubscriptionType.TICKER.getType()))
            .map(it -> this.<TickerBinanceWebsocketTransaction>readTransaction(it, TICKER_TYPE, "ticker"))
            .filter(transaction -> BinanceAdapters.adaptSymbol(transaction.getData().getSymbol(), instrument instanceof FuturesContract)
                    .equals(instrument)).map(transaction -> transaction.getData().getTicker());
  }

  private Observable<BinanceBookTicker> rawBookTickerStream(Instrument instrument) {
    return service.subscribeChannel(channelFromCurrency(instrument, BinanceSubscriptionType.BOOK_TICKER.getType()))
            .map(it -> this.<BookTickerBinanceWebSocketTransaction>readTransaction(it, BOOK_TICKER_TYPE, "book ticker"))
            .filter(transaction -> BinanceAdapters.adaptSymbol(transaction.getData().getTicker().getSymbol(),
                    instrument instanceof FuturesContract).equals(instrument)).map(transaction -> transaction.getData().getTicker());
  }
  ...
  private final class OrderBookSubscription implements Disposable {
    ...
    private OrderBookSubscription(Observable<DepthBinanceFutureWebSocketTransaction> deltasObservable, Instrument currencyPair) {
      this.deltasObservable = deltasObservable;
      this.currencyPair = currencyPair;
    }

    public Observable<OrderBook> connect() {
      if (isDisposed()) throw new IllegalStateException("Disposed before, use a new instance to connect next time.");

      disposables.add(asyncInitializeOrderBookSnapshot());

      deltasObservable.doOnNext(delta -> {
      ...
      }).filter(delta -> isBookInitialized()).map(delta -> getBook()).doFinally(() -> dispose()).subscribe(booksSubject);
    ...
    private Disposable asyncInitializeOrderBookSnapshot() {
      ...
      return deltasObservable.firstOrError().observeOn(bookSnapshotsScheduler)
              .flatMap(delta -> fetchSingleBinanceOrderBookUpdatedAfter(delta)).subscribe(binanceBook -> {
                final OrderBook convertedBook = BinanceFutureMarketDataService.convertOrderBook(binanceBook, currencyPair);

                synchronized (bookIntegrityMonitor) {
                  book = convertedBook;
                  final List<DepthBinanceFutureWebSocketTransaction> applicableBookPatches = deltasBuffer.stream()
                          .filter(delta -> delta.getLastUpdateId() >= binanceBook.lastUpdateId).collect(Collectors.toList());
                  ...
                  for (DepthBinanceFutureWebSocketTransaction d : deltasBuffer) {
                    LOG.trace("db uidp {} U {} u {} E {} pu {} T {}", finalUpdateidPrev, d.getFirstUpdateId(), d.getLastUpdateId(),
                            d.getEventTime().getTime(), d.getFinalUpdateid(), Long.parseLong(d.getTransactionTime()));
                  }
                  deltasBuffer.clear();
                  ...
                  for (DepthBinanceFutureWebSocketTransaction applicableBookPatch : applicableBookPatches) {
                    ...
                    if (!checkPass && applicableBookPatch.getFirstUpdateId() <= binanceBook.lastUpdateId) checkPass = true;
                    if (checkPass) if (!appendDelta(applicableBookPatch)) {
                      disposables.add(asyncInitializeOrderBookSnapshot());
                    }
                  }
                }
    ...
    private boolean appendDelta(DepthBinanceFutureWebSocketTransaction delta) {
      LOG.trace("appendDelta before sync idPrev {} U {} u {} E {} pu {} T {}", finalUpdateidPrev, delta.getFirstUpdateId(),
              delta.getLastUpdateId(), delta.getEventTime().getTime(), delta.getFinalUpdateid(),
              Long.parseLong(delta.getTransactionTime()));

      synchronized (bookIntegrityMonitor) {
        LOG.trace("appendDelta before check idPrev {} U {} u {} E {} pu {} T {}", finalUpdateidPrev, delta.getFirstUpdateId(),
                delta.getLastUpdateId(), delta.getEventTime().getTime(), delta.getFinalUpdateid(),
                Long.parseLong(delta.getTransactionTime()));
        if (finalUpdateidPrev != 0 && finalUpdateidPrev != delta.getFinalUpdateid()) {
          LOG.trace("Orderbook snapshot for {} out of date (idPrev={}, U={}, u={}).", currencyPair, finalUpdateidPrev,
                  delta.getFirstUpdateId(), delta.getLastUpdateId());
          return false;
        } else {
    ...
    private Single<BinanceOrderbook> fetchSingleBinanceOrderBookUpdatedAfter(final DepthBinanceFutureWebSocketTransaction delta) {
      return Single.fromCallable(() -> {
        ...
        do {
          attemptNum++;
          // Get a snapshot.
          LOG.info("Fetching initial orderbook snapshot for {}, attempt #{}", currencyPair, attemptNum);
          ...
          // time.
          LOG.info("initial book snapshot for {} (lastUpdateId={}, U={}, u={}).", currencyPair, snapshot.lastUpdateId,
                  delta.getFirstUpdateId(), delta.getLastUpdateId());
        } while (snapshot.lastUpdateId < delta.getFirstUpdateId());
        return snapshot;
      }).doOnError(error -> LOG.error("Failed to fetch initial order book for " + currencyPair, error));
    }

    private BinanceOrderbook fetchBinanceOrderBook(Instrument currencyPair) throws IOException, InterruptedException {
      try {
        onApiCall.run();
  ...
  private Observable<DepthBinanceFutureWebSocketTransaction> rawOrderBookUpdates(Instrument instrument) {
    return service.subscribeChannel(channelFromCurrency(instrument, BinanceSubscriptionType.DEPTH.getType()))
            .map(it -> this.<DepthBinanceFutureWebSocketTransaction>readTransaction(it, DEPTH_TYPE, "order book"))
            .filter(transaction -> BinanceAdapters.adaptSymbol(transaction.getData().getSymbol(), instrument instanceof FuturesContract)
                    .equals(instrument)).map(transaction -> transaction.getData());
//            .map(BinanceWebsocketTransaction::getData).filter(data -> data.getCurrencyPair().equals(instrument));
  }

  private Observable<OrderBook> createOrderBookObservable(Instrument currencyPair) {
    return new OrderBookSubscription(orderBookRawUpdatesSubscriptions.get(currencyPair), currencyPair).connect();
  }

  private Observable<BinanceRawTrade> rawTradeStream(Instrument instrument) {
    return service.subscribeChannel(channelFromCurrency(instrument, BinanceSubscriptionType.TRADE.getType()))
            .map(it -> this.<TradeBinanceWebsocketTransaction>readTransaction(it, TRADE_TYPE, "trade"))
            .filter(transaction -> BinanceAdapters.adaptSymbol(transaction.getData().getSymbol(), instrument instanceof FuturesContract)
                    .equals(instrument)).map(transaction -> transaction.getData().getRawTrade());
  }
  ...
  private <T> BinanceWebsocketTransaction<T> readTransaction(JsonNode node, JavaType type, String transactionType) {
    try {
      return mapper.readValue(mapper.treeAsTokens(node), type);
    } catch (IOException e) {
      throw new ExchangeException(String.format("Unable to parse %s transaction", transactionType), e);
    }
  }

  private Stream<OrderBookUpdate> extractOrderBookUpdates(Instrument currencyPair,
                                                          DepthBinanceFutureWebSocketTransaction depthTransaction) {
    ...
    LOG.trace("dt U {} u {} E {} pu {} T {}", delta.getFirstUpdateId(), delta.getLastUpdateId(), delta.getEventTime().getTime(),
            delta.getFinalUpdateid(), Long.parseLong(delta.getTransactionTime()));
  ...
  private static JavaType getBookTickerType() {
    return getObjectMapper().getTypeFactory()
            .constructType(new TypeReference<BinanceWebsocketTransaction<BookTickerBinanceWebSocketTransaction>>() {
            });
  }
  ...
  private static JavaType getDepthType() {
    return getObjectMapper().getTypeFactory()
            .constructType(new TypeReference<BinanceWebsocketTransaction<DepthBinanceFutureWebSocketTransaction>>() {
            });
  }

  private static JavaType getKlineType() {
    return getObjectMapper().getTypeFactory()
            .constructType(new TypeReference<BinanceWebsocketTransaction<KlineBinanceWebSocketTransaction>>() {
            });
  }
}
