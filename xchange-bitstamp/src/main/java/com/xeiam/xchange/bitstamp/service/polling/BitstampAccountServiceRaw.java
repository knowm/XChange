...
public class BitstampAccountServiceRaw extends BitstampBasePollingService {
  ...
  public BitstampDepositAddress getBitstampBitcoinDepositAddress() throws IOException {
    ...
    final BitstampDepositAddress response = bitstampAuthenticated.getBitcoinDepositAddress(exchange.getExchangeSpecification().getApiKey(), signatureCreator, BitstampUtils.getNonce());
    if (response.getError() != null) {
  ...
  public BitstampRippleDepositAddress getRippleDepositAddress() throws IOException {
    ...
    return bitstampAuthenticated.getRippleDepositAddress(exchange.getExchangeSpecification().getApiKey(), signatureCreator, BitstampUtils.getNonce());
  }

  /**
   * @return true if withdrawal was successfull. Note that due to a bug on
  ...
  public boolean withdrawToRipple(BigDecimal amount, String currency, String rippleAddress) throws IOException {
    ...
    return bitstampAuthenticated.withdrawToRipple(exchange.getExchangeSpecification().getApiKey(), signatureCreator, BitstampUtils.getNonce(), amount, currency, rippleAddress);
  }

  public List<DepositTransaction> getUnconfirmedDeposits() throws IOException {

    final List<DepositTransaction> response = Arrays.asList(bitstampAuthenticated.getUnconfirmedDeposits(exchange.getExchangeSpecification().getApiKey(), signatureCreator, BitstampUtils.getNonce()));
    return response;
  }

  public List<WithdrawalRequest> getWithdrawalRequests() throws IOException {

    final List<WithdrawalRequest> response = Arrays.asList(bitstampAuthenticated.getWithdrawalRequests(exchange.getExchangeSpecification().getApiKey(), signatureCreator, BitstampUtils.getNonce()));
    return response;
  }
