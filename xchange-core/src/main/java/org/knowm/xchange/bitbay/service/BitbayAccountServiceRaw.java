...
public class BitbayAccountServiceRaw extends BitbayBaseService {
  ...
  public BitbayBaseResponse transfer(Currency currency, BigDecimal quantity, String address) {
    BitbayBaseResponse resp = bitbayAuthenticated
        .transfer(apiKey, sign, exchange.getNonceFactory(), currency.getCurrencyCode(), quantity.toString(), address);
  ...
  public BitbayBaseResponse withdraw(Currency currency, BigDecimal quantity, String account, boolean express, String bicOrSwiftCode) {
    BitbayBaseResponse resp = bitbayAuthenticated
        .withdraw(apiKey, sign, exchange.getNonceFactory(), currency.getCurrencyCode(), quantity.toString(), account, Boolean.toString(express),
            bicOrSwiftCode);
    if (resp.getMessage() != null)
      throw new ExchangeException(resp.getMessage());
    return resp;
  ...
  public List<FundingRecord> history(Currency currency, int limit) {
    ...
    for (Map map : history) {
      try {
        ...
        res.add(new FundingRecord(null, dateFormat.parse(map.get("time").toString()), Currency.getInstance(map.get("currency").toString()),
            new BigDecimal(map.get("amount").toString()), map.get("id").toString(), null, type, FundingRecord.Status.COMPLETE, null, null,
