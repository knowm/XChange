...
public class DSXTradeServiceRaw extends DSXBaseService {
  ...
  public DSXTradeResult tradeDSX(DSXOrder order) throws IOException {
    ...
    DSXTradeReturn ret = dsx
        .trade(apiKey, signatureCreator, exchange.getNonceFactory(), order.getType(), order.getRate(), order.getAmount(), pair, order.getOrderType());
    checkResult(ret);
   ...
   * @throws IOException
   */
  public Map<Long, DSXTradeHistoryResult> getDSXTradeHistory(Integer count, Long fromId, Long endId, DSXAuthenticatedV2.SortOrder order, Long since,
      Long end, String pair) throws IOException {

    DSXTradeHistoryReturn dsxTradeHistory = dsx
        .tradeHistory(apiKey, signatureCreator, exchange.getNonceFactory(), count, fromId, endId, order, since, end, pair);
   ...
   * @throws IOException
   */
  public Map<Long, DSXTransHistoryResult> getDSXTransHistory(Integer count, Long fromId, Long endId, DSXAuthenticatedV2.SortOrder order, Long since,
      Long end, DSXTransHistoryResult.Type type, DSXTransHistoryResult.Status status, String currency) throws IOException {

    DSXTransHistoryReturn dsxTransHistory = dsx
        .transHistory(apiKey, signatureCreator, exchange.getNonceFactory(), count, fromId, endId, order, since, end, type, status, currency);
   ...
   * @throws IOException
   */
  public Map<Long, DSXOrderHistoryResult> getDSXOrderHistory(Long count, Long fromId, Long endId, DSXAuthenticatedV2.SortOrder order, Long since,
      Long end, String pair) throws IOException {

    DSXOrderHistoryReturn dsxOrderHistory = dsx
        .orderHistory(apiKey, signatureCreator, exchange.getNonceFactory(), count, fromId, endId, order, since, end, pair);
