...
public class WexTradeServiceRaw extends WexBaseService {
  ...
  public WexPlaceOrderResult placeBTCEOrder(WexOrder order) throws IOException {
    ...
    WexPlaceOrderReturn ret = btce
        .Trade(apiKey, signatureCreator, exchange.getNonceFactory(), pair, order.getType(), order.getRate(), order.getAmount());
    checkResult(ret);
  ...
  public Map<Long, WexTradeHistoryResult> getBTCETradeHistory(Long from, Long count, Long fromId, Long endId, WexAuthenticated.SortOrder order,
    ...
    WexTradeHistoryReturn btceTradeHistory = btce
        .TradeHistory(apiKey, signatureCreator, exchange.getNonceFactory(), from, count, fromId, endId, order, since, end, pair);
  ...
  public Map<Long, WexTransHistoryResult> getBTCETransHistory(Long from, Long count, Long fromId, Long endId, WexAuthenticated.SortOrder order,
    ...
    WexTransHistoryReturn btceTransHistory = btce
        .TransHistory(apiKey, signatureCreator, exchange.getNonceFactory(), from, count, fromId, endId, order, since, end);
