...
public class CoinEggAdapters {
  ...
  public static Ticker adaptTicker(CoinEggTicker coinEggTicker, CurrencyPair currencyPair) {
    ...
    Ticker ticker = new Ticker.Builder().currencyPair(currencyPair).last(last).bid(bid).ask(ask).high(high).low(low).volume(volume).build();
  ...
  public static LimitOrder adaptOrder(CoinEggOrder order, OrderType type, CurrencyPair currencyPair) {
    ...
    LimitOrder limitOrder = new LimitOrder.Builder(type, currencyPair).originalAmount(quantity).limitPrice(price).build();
  ...
  public static OrderBook adaptOrders(CoinEggOrders coinEggOrders, CurrencyPair currencyPair) {
    ...
    List<LimitOrder> asks = Stream.of(coinEggOrders.getAsks()).map(order -> adaptOrder(order, OrderType.ASK, currencyPair))
                                  .collect(Collectors.toList());

    List<LimitOrder> bids = Stream.of(coinEggOrders.getBids()).map(order -> adaptOrder(order, OrderType.BID, currencyPair))
                                  .collect(Collectors.toList());
  ...
  public static Trade adaptTrade(CoinEggTrade coinEggTrade, CurrencyPair currencyPair) {
    return new Trade.Builder().currencyPair(currencyPair).id(String.valueOf(coinEggTrade.getTransactionID())).type(coinEggTrade.getOrderType())
                              .price(coinEggTrade.getPrice()).originalAmount(coinEggTrade.getAmount()).build();
  }

  public static Trades adaptTrades(CoinEggTrade[] coinEggTrades, CurrencyPair currencyPair) {
    List<Trade> trades = Stream.of(coinEggTrades).map(t -> adaptTrade(t, currencyPair)).collect(Collectors.toList());
  ...
  public static UserTrades adaptTradeHistory(CoinEggTradeView coinEggTradeView) {
    ...
    Trade trade = new Trade.Builder()
        //.currencyPair(null)
        .id(String.valueOf(coinEggTradeView.getID())).type(coinEggTradeView.getType() == Type.BUY ? OrderType.ASK : OrderType.BID)
        .price(coinEggTradeView.getPrice()).originalAmount(coinEggTradeView.getAmountOriginal()).timestamp(coinEggTradeView.getDateTime()).build();
