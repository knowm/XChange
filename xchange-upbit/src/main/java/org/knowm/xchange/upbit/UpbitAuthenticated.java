...
public interface UpbitAuthenticated extends Upbit {
    ...
    @POST
    @Path("orders")
    UpbitOrderResponse limitOrder(@HeaderParam("Authorization") ParamsDigest signatureCreator, UpbitOrderRequest upbitOrderRequest)
        throws IOException, UpbitException;

    @DELETE
    @Path("order")
    UpbitOrderResponse cancelOrder(@HeaderParam("Authorization") ParamsDigest signatureCreator, @QueryParam("uuid") String uuid)
        throws IOException, UpbitException;

    @GET
    @Path("order")
    UpbitOrderResponse getOrder(@HeaderParam("Authorization") ParamsDigest signatureCreator, @QueryParam("uuid") String uuid)
        throws IOException, UpbitException;
}
