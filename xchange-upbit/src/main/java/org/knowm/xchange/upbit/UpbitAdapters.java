...
public final class UpbitAdapters {
	...
	public static OrderBook adaptOrderBook(UpbitOrderBooks upbitOrderBooks) {
		...
		CurrencyPair currencyPair = new CurrencyPair(Currency.getInstance(market.split("-")[0]),
				Currency.getInstance(market.split("-")[1]));
	...
	private static Map<OrderType, List<LimitOrder>> adaptMarketOrderToLimitOrder(UpbitOrderBookData[] upbitOrders,
	...
	public static Ticker adaptTicker(UpbitTickers tickers) {
		...
		CurrencyPair currencyPair = new CurrencyPair(Currency.getInstance(market.split("-")[0]),
				Currency.getInstance(market.split("-")[1]));
		final Date date = DateUtils.fromMillisUtc(Long.valueOf(ticker.getTimestamp().longValue()) * 1000);
		return new Ticker.Builder().currencyPair(currencyPair).high(ticker.getHigh_price()).low(ticker.getLow_price())
				.last(ticker.getTrade_price()).volume(ticker.getTrade_volume()).open(ticker.getOpening_price()).timestamp(date).build();
	}
	...
	private static Trade adaptTrade(UpbitTrade trade, CurrencyPair currencyPair) {
		...
		return new Trade(orderType, trade.getTradeVolume(), currencyPair, trade.getTradePrice(),
