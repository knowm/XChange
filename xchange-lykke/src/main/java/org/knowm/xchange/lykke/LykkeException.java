...
public class LykkeException extends HttpStatusExceptionSupport{
    ...
    public LykkeException(
            @JsonProperty("Error") LykkeError error) {
        this.error = error;
    }
    ...
    public String getMessage() {
        return String.format("%s - %s (HTTP status code: %d)", error.getCode(), error.getMessage(), getHttpStatusCode());
    }

    @Override
    public String toString() {
        return "LykkeException{" +
                "error=" + error +
                '}';
    }
}
