...
//    private static SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
    ...
    public static OrderBook adaptOrderBook(List<LykkeOrderBook> lykkeOrderBook, CurrencyPair currencyPair) throws IOException{
        ...
        List<LimitOrder> limitOrders = new ArrayList<>();
                ...
                for (LykkePrices lykkePrices : lykkeOrderBook.getPrices()) {
                    limitOrders.add(new LimitOrder(
                            orderType,
                            new BigDecimal(Math.abs(lykkePrices.getVolume())).setScale(8, RoundingMode.HALF_EVEN).stripTrailingZeros(),
                            currencyPair,
                            null,
                            DateUtils.fromISO8601DateString(lykkeOrderBook.getTimestamp()),
                            new BigDecimal(lykkePrices.getPrice()).setScale(8, RoundingMode.HALF_EVEN).stripTrailingZeros()
                    ));
                }
    ...
    public static List<LimitOrder> adaptOpenOrders(List<CurrencyPair> currencyPairList, List<LykkeOrder> lykkeOrders)
            throws IOException{
        List<LimitOrder> limitOrders = new ArrayList<>();
    ...
    public static LimitOrder adaptLimitOrder(List<CurrencyPair> currencyPairList, LykkeOrder lykkeOrder)
            throws IOException {

            return new LimitOrder(
                    getOrderTypeFromVolumeSign(lykkeOrder.getVolume()),
                    new BigDecimal(Math.abs(lykkeOrder.getVolume())).setScale(8,RoundingMode.HALF_EVEN).stripTrailingZeros(),
                    adaptToCurrencyPair(currencyPairList, lykkeOrder.getAssetPairId()),
                    lykkeOrder.getId(),
                    DateUtils.fromISO8601DateString(lykkeOrder.getCreatedAt()),//formatter.parse(lykkeOrder.getCreatedAt())
                    new BigDecimal(lykkeOrder.getPrice()).setScale(8,RoundingMode.HALF_EVEN).stripTrailingZeros());

    }

    /****Adapter for LykkeTradeHistory****/

    public static List<UserTrade> adaptUserTrades(List<CurrencyPair> currencyPairList, List<LykkeOrder> tradeHistoryList)
            throws IOException{
    ...
    private  static UserTrade adaptUserTrade(List<CurrencyPair> currencyPairList, LykkeOrder tradeHistory)
            throws IOException{

            return new UserTrade(
                    getOrderTypeFromVolumeSign(tradeHistory.getVolume()),
                    new BigDecimal(Math.abs(tradeHistory.getVolume())).setScale(8,RoundingMode.HALF_EVEN).stripTrailingZeros(),
                    adaptToCurrencyPair(currencyPairList, tradeHistory.getAssetPairId()),
                    new BigDecimal(tradeHistory.getPrice()).setScale(8,RoundingMode.HALF_EVEN).stripTrailingZeros(),
    ...
    public static AccountInfo adaptAccountInfo(List<LykkeWallet> lykkeWallets){
       ...
       for (LykkeWallet lykkeWallet : lykkeWallets){
           balances.add(new Balance(
                   ...
                   new BigDecimal(lykkeWallet.getBalance()).setScale(8,RoundingMode.HALF_EVEN).stripTrailingZeros(),
                   new BigDecimal(lykkeWallet.getBalance()-lykkeWallet.getReserved()).setScale(8,RoundingMode.HALF_EVEN).stripTrailingZeros(),
                   new BigDecimal(lykkeWallet.getReserved()).setScale(8,RoundingMode.HALF_EVEN).stripTrailingZeros()));
       }
    ...
    private static CurrencyPair adaptToCurrencyPair(List<CurrencyPair> currencyPairList, String assetPair) throws IOException{
