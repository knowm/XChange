...
public class FtxStreamingAdapters {
    ...
    private final static ObjectMapper mapper = StreamingObjectMapperHelper.getObjectMapper();
    ...
    public static OrderBook adaptOrderbookMessage(OrderBook orderBook, Instrument instrument, JsonNode jsonNode){
        ...
        Streams.stream(jsonNode)
                ...
                .forEach(
                        message -> {
                            ...
                            if(message.getAction().equals("partial")){
                                message.getAsks().forEach(ask->{
                                    orderBook.getAsks().add(new LimitOrder.Builder(Order.OrderType.ASK,instrument).limitPrice(ask.get(0)).originalAmount(ask.get(1)).build());
                                });

                                message.getBids().forEach(bid ->{
                                    orderBook.getBids().add(new LimitOrder.Builder(Order.OrderType.BID,instrument).limitPrice(bid.get(0)).originalAmount(bid.get(1)).build());
                                });
                            }else{
                                message.getAsks().forEach(ask->
                                        orderBook.update(new LimitOrder.Builder(Order.OrderType.ASK,instrument).limitPrice(ask.get(0)).originalAmount(ask.get(1)).build())
                                );
                                message.getBids().forEach(bid->
                                        orderBook.update(new LimitOrder.Builder(Order.OrderType.BID,instrument).limitPrice(bid.get(0)).originalAmount(bid.get(1)).build())
                                );
                            }
        ...
        return new OrderBook(Date.from(Instant.now()), Lists.newArrayList(orderBook.getAsks()),Lists.newArrayList(orderBook.getBids()),true);
    }
    ...
    public static Long getOrderbookChecksum(List<LimitOrder> asks,List<LimitOrder> bids){
        ...
        byte[] toBytes = data.toString().getBytes();
