...
import org.knowm.xchange.mexbt.dto.account.MeXBTBalance;
import org.knowm.xchange.mexbt.dto.account.MeXBTBalanceResponse;
import org.knowm.xchange.mexbt.dto.account.MeXBTDepositAddress;
import org.knowm.xchange.mexbt.dto.account.MeXBTDepositAddressesResponse;
import org.knowm.xchange.mexbt.dto.account.MeXBTOpenOrder;
import org.knowm.xchange.mexbt.dto.account.MeXBTOpenOrdersInfo;
...
import org.knowm.xchange.mexbt.dto.account.MeXBTUserTrade;
...
public final class MeXBTAdapters {
  ...
  public static Ticker adaptTicker(CurrencyPair currencyPair, MeXBTTicker meXBTTicker) {
    ...
    return new Ticker.Builder().currencyPair(currencyPair).last(meXBTTicker.getLast()).bid(meXBTTicker.getBid()).ask(meXBTTicker.getAsk()).high(meXBTTicker.getHigh()).low(meXBTTicker.getLow()).volume(
        meXBTTicker.getVolume24Hour()).build();
  }
  ...
  public static Wallet adaptWallet(MeXBTBalanceResponse balanceResponse) {
    ...
    for (MeXBTBalance mxbtBalance : balanceResponse.getCurrencies()) {
      Balance balance = new Balance.Builder().currency(Currency.getInstance(mxbtBalance.getName())).available(mxbtBalance.getBalance()).frozen(mxbtBalance.getHold()).build();
      wallet.add(balance);
    }
  ...
  public static OpenOrders adaptOpenOrders(MeXBTOpenOrdersResponse openOrdersResponse) {
    ...
    for (MeXBTOpenOrdersInfo openOrdersInfo : openOrdersResponse.getOpenOrdersInfo()) {
      ...
      for (MeXBTOpenOrder openOrder : openOrdersInfo.getOpenOrders()) {
        LimitOrder limitOrder = new LimitOrder.Builder(adaptSide(openOrder.getSide()), currencyPair).id(String.valueOf(openOrder.getServerOrderId())).timestamp(openOrder.getReceiveTime()).limitPrice(
            openOrder.getPrice()).tradableAmount(openOrder.getQtyRemaining()).build();
