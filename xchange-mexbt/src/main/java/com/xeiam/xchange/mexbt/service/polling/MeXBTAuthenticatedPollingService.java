...
public abstract class MeXBTAuthenticatedPollingService extends MeXBTBasePollingService {
  ...
  protected MeXBTAuthenticatedPollingService(Exchange exchange) {
    super(exchange);
    this.meXBTAuthenticated = RestProxyFactory.createProxy(MeXBTAuthenticated.class, (String) exchange.getExchangeSpecification().getExchangeSpecificParametersItem(MeXBTExchange.PRIVATE_API_URI_KEY));
    this.apiKey = exchange.getExchangeSpecification().getApiKey();
    this.nonceFactory = exchange.getNonceFactory();
    this.meXBTDigest = MeXBTDigest.createInstance(exchange.getExchangeSpecification().getSecretKey(), exchange.getExchangeSpecification().getUserName(), exchange.getExchangeSpecification()
        .getApiKey());
  }
