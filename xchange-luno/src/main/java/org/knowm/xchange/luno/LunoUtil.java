...
public class LunoUtil {
    ...
    public static String requestType(String lunoCurrency) {
        switch (lunoCurrency) {
        case "ZAR": return "ZAR_EFT";
        case "NAD": return "NAD_EFT";
        case "KES": return "KES_MPESA";
        case "MYR": return "MYR_IBG";
        case "IDR": return "IDR_LLG";
        default: throw new ExchangeException("Luno does not support withdrawals for " + lunoCurrency);
        }
    }

    /**
     * convert luno pair to xchange pair
     * i.e. XBTZAR -> BTC/ZAR
     * we assume, the pair has two currencies with 3 chars length each <- not a very clean approach
     * @param lunoPair
     * @return
     */
