...
public interface LunoAuthenticated extends Luno {
  ...
  /**
   * Create an additional account for the specified currency. You must be verified to trade currency
   * in order to be able to create an account. A user has a limit of 4 accounts per currency.
   *
   * @param auth
   * @param currency required - The currency code for the account you want to create e.g. XBT, IDR,
   *     MYR, ZAR
  ...
  @GET
  @Path("balance")
  LunoBalance balance(@HeaderParam("Authorization") ParamsDigest auth)
      throws IOException, LunoException;
  ...
  LunoAccountTransactions transactions(
      @HeaderParam("Authorization") ParamsDigest auth,
  ...
  LunoPendingTransactions pendingTransactions(
      @HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String id)
      throws IOException, LunoException;
   ...
   * @param type required - "BID" for a bid (buy) limit order or "ASK" for an ask (sell) limit
   *     order.
   * @param volume required - Amount of Bitcoin to buy or sell as a decimal string in units of BTC
   *     e.g. "1.423".
   ...
   * @param counterVolume required, if type is "BUY" - For a "BUY" order: amount of local currency
   *     (e.g. ZAR, MYR) to spend as a decimal string in units of the local currency e.g. "100.50".
   * @param baseVolume required, if type is "SELL" - For a "SELL" order: amount of Bitcoin to sell
   *     as a decimal string in units of BTC e.g. "1.423".
   * @param baseAccountId optional - The base currency account to use in the trade.
   * @param counterAccountId optional - The counter currency account to use in the trade.
   * @return
   * @throws IOException
  ...
  @POST
  @Path("stoporder")
  LunoBoolean stopOrder(
      @HeaderParam("Authorization") ParamsDigest auth, @FormParam("order_id") String orderId)
      throws IOException, LunoException;
  ...
  @GET
  @Path("orders/{id}")
  Order getOrder(@HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String orderId)
      throws IOException, LunoException;
  ...
  @Path("fee_info")
  LunoFeeInfo feeInfo(
      @HeaderParam("Authorization") ParamsDigest auth, @QueryParam("pair") String pair)
      throws IOException, LunoException;
   ...
   * @param address optional - Specific Bitcoin address to retrieve. If not provided, the default
  ...
  /**
   * Allocates a new receive address to your account. There is a rate limit of 1 address per hour,
  ...
  @POST
  @Path("funding_address")
  LunoFundingAddress createFundingAddress(
      @HeaderParam("Authorization") ParamsDigest auth, @FormParam("asset") String asset)
      throws IOException, LunoException;
  ...
  @GET
  @Path("withdrawals")
  LunoWithdrawals withdrawals(@HeaderParam("Authorization") ParamsDigest auth)
      throws IOException, LunoException;
   ...
   * @param beneficiaryId optional - The beneficiary ID of the bank account the withdrawal will be
   *     paid out to. This parameter is required if you have multiple bank accounts. Your bank
   *     account beneficiary ID can be found by clicking on the beneficiary name on the
   *     Beneficiaries page [https://www.luno.com/wallet/beneficiaries].
   * @return
   * @throws IOException
   * @throws LunoException
   */
  ...
  Withdrawal getWithdrawal(
      @HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String withdrawalId)
      throws IOException, LunoException;
   ...
   * @param description optional - Description for the transaction to record on the account
   *     statement.
   * @param message optional - Message to send to the recipient. This is only relevant when sending
   *     to an email address.
   * @return
   * @throws IOException
   * @throws LunoException
   ...
   * @param pair required - Currency pair to trade e.g. XBTZAR, XBTMYR. The pair can also be flipped
   *     if you want to buy or sell the counter currency (e.g. ZARXBT).
   * @return
   * @throws IOException
   * @throws LunoException
   */
  ...
  @Path("quotes/{id}")
  LunoQuote getQuote(
      @HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String quoteId)
      throws IOException, LunoException;

  /**
   * Exercise a quote to perform the trade. If there is sufficient balance available in your
  ...
  @PUT
  @Path("quotes/{id}")
  LunoQuote exerciseQuote(
      @HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String quoteId)
      throws IOException, LunoException;

  /**
   * Discard a quote. Once a quote has been discarded, it cannot be exercised even if it has not
   * expired yet.
   *
   * @param auth
