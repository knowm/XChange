...
public class CoinbaseProAccountServiceRaw extends CoinbaseProBaseService {
  ...
  public CoinbaseProTransfers transfers(
      String accountId, String profileId, int limit, String after) throws IOException {
    return decorateApiCall(() ->
  ...
  public CoinbaseProWebsocketAuthData getWebsocketAuthData()
    ...
    JsonNode json = decorateApiCall(() ->
            coinbasePro.getVerifyId(apiKey, digest, timestamp, passphrase))
            .withRateLimiter(rateLimiter(PRIVATE_REST_ENDPOINT_RATE_LIMITER))
            .call();
