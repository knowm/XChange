...
public class OkxStreamingMarketDataService implements StreamingMarketDataService {
    ...
    public void unsubscribe(Instrument instrument, String subscriptionType) {
        ...
        OkxSubscribeMessage.SubscriptionTopic topic = new OkxSubscribeMessage.SubscriptionTopic(subscriptionType, null, null, instId);
...
//        instId += "-"+subscriptionType;
    ...
    public Observable<OrderBook> getOrderBook(Instrument instrument, BigDecimal multiplier, String args) {
        ...
        return service
                ...
                .flatMap(jsonNode -> {
                    ...
                    if ("snapshot".equalsIgnoreCase(action)) {
                        List<OkexOrderbook> okexOrderbooks = mapper.treeToValue(jsonNode.get("data"), mapper.getTypeFactory().constructCollectionType(List.class, OkexOrderbook.class));
                        OrderBook orderBook = OkexAdapters.adaptOrderBook(okexOrderbooks, instrument, multiplier);
                        orderBookMap.put(instId, orderBook);
                        return Observable.just(orderBook);
                    } else if ("update".equalsIgnoreCase(action)) {
                        OrderBook orderBook = orderBookMap.getOrDefault(instId, null);
    ...
    public Observable<OrderBook> getOrderBookFutures(Instrument instrument, BigDecimal multiplier, String args) {
        String channelName = args.length() >= 1 ? args : "books";
        String instId = OkexAdapters.adaptInstrumentId(instrument);
        String SubscriptionName = "";
        if (args.length() >= 1) {
