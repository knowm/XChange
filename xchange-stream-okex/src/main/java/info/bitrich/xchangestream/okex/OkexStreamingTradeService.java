...
public class OkexStreamingTradeService implements StreamingTradeService {
  ...
  public Observable<UserTrade> getUserTrades(Instrument instrument, Object... args) {
    ...
    message
        .getArgs()
        .add(
            new OkexSubscribeMessage.SubscriptionTopic(
                channelName,
                OkexInstType.ANY,
                null,
                OkexAdapters.adaptInstrumentToOkexInstrumentId(instrument)));

    return service
        .subscribeChannel(channelName, message)
        .flatMap(
            jsonNode -> {
              ...
              return Observable.fromIterable(
                  OkexAdapters.adaptUserTrades(okexOrderDetails).getUserTrades());
            });
  }
}
