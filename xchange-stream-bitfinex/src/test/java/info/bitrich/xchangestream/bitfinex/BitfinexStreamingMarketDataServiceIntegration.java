...
public class BitfinexStreamingMarketDataServiceIntegration extends BitfinexStreamingExchangeIT {
  ...
  void order_book() {
    Observable<OrderBook> observable =
        exchange.getStreamingMarketDataService().getOrderBook(CurrencyPair.BTC_USDT, 25);
    ...
    assertThat(orderBook.getAsks().get(0).getInstrument()).isEqualTo(CurrencyPair.BTC_USDT);
    assertThat(orderBook).hasNoNullFieldsOrProperties();
    assertThat(orderBook.getBids()).hasSize(25);
    assertThat(orderBook.getAsks()).hasSize(25);
