...
public class TaurusTradeServiceRaw extends TaurusBaseService {
  ...
  public TaurusOrder[] getTaurusOpenOrders() throws IOException {
    return taurusAuthenticated.getOpenOrders(
        exchange.getExchangeSpecification().getApiKey(),
        signatureCreator,
        exchange.getNonceFactory());
  }

  public TaurusOrder sellTaurusOrder(BigDecimal originalAmount, BigDecimal price)
      throws IOException {
    return taurusAuthenticated.sell(
        exchange.getExchangeSpecification().getApiKey(),
        signatureCreator,
        exchange.getNonceFactory(),
        originalAmount,
  ...
  public TaurusOrder buyTaurusOrder(BigDecimal originalAmount, BigDecimal price)
      throws IOException {
    return taurusAuthenticated.buy(
        exchange.getExchangeSpecification().getApiKey(),
        signatureCreator,
        exchange.getNonceFactory(),
        originalAmount,
  ...
  public boolean cancelTaurusOrder(String orderId) throws IOException {
    return taurusAuthenticated.cancelOrder(
        exchange.getExchangeSpecification().getApiKey(),
        signatureCreator,
        exchange.getNonceFactory(),
        orderId);
  ...
  public TaurusUserTransaction[] getTaurusUserTransactions(
      Integer offset, Integer limit, TradeHistoryParamsSorted.Order sort) throws IOException {
    return taurusAuthenticated.getUserTransactions(
        exchange.getExchangeSpecification().getApiKey(),
        signatureCreator,
        exchange.getNonceFactory(),
        limit,
