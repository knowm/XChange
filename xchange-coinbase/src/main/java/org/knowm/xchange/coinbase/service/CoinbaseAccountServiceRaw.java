...
class CoinbaseAccountServiceRaw extends CoinbaseBaseService {
  ...
  /**
   * Authenticated resource that lets you update account settings for the current user. Use {@link
   * #getCoinbaseUsers()} to retrieve the current user first.
   *
  ...
  public CoinbaseUser updateCoinbaseUser(CoinbaseUser user) throws IOException {
    ...
    final CoinbaseUser updatedUser =
        coinbase.updateUser(
            user.getId(),
            user,
            exchange.getExchangeSpecification().getApiKey(),
   ...
   *
   * @return A {@code CoinbaseAmount} wrapper around a {@code CoinbaseMoney} object representing the
  ...
  /**
   * Authenticated resource which returns all related changes to an account. This is an alternative
   * to the {@code getCoinbaseTransactions} API call. It is designed to be faster and provide more
   * detail so you can generate an overview/summary of individual account changes. This is a paged
   ...
   * Authenticated resource which returns all related changes to an account. This is an alternative
   * to the {@code getCoinbaseTransactions} API call. It is designed to be faster and provide more
   * detail so you can generate an overview/summary of individual account changes.
   *
   * @param page Optional parameter to request a desired page of results. Will return page 1 if the
   *     supplied page is null or less than 1.
   * @return The current user, balance, and the most recent account changes.
  ...
  /**
   * Authenticated resource which returns the user’s most recent transactions. Sorted in descending
   * order by creation date. This is a paged resource and will return the first page by default.
   *
  ...
  public CoinbaseTransactions getCoinbaseTransactions(Integer page) throws IOException {
    ...
    final CoinbaseTransactions transactions =
  ...
  /**
   * Authenticated resource which lets a user complete a money request. Money requests can only be
   * completed by the sender (not the recipient). Remember that the sender in this context is the
   * user who is sending money (not sending the request itself).
   *
   * @param transactionId
   * @return The {@code CoinbaseTransaction} representing the completed {@code
   *     CoinbaseSendMoneyRequest}.
   * @throws IOException
   * @see <a
  ...
  public CoinbaseTransaction completeCoinbaseRequest(String transactionId) throws IOException {
    ...
    final CoinbaseTransaction response =
        coinbase.completeRequest(
            transactionId,
            exchange.getExchangeSpecification().getApiKey(),
            signatureCreator,
            exchange.getNonceFactory());
  ...
  /**
   * Authenticated resource which returns a merchant’s orders that they have received. Sorted in
   * descending order by creation date. This is a paged resource and will return the first page by
   * default, use {@link #getCoinbaseOrders(Integer page)} to retrieve additional pages.
   *
   * @return
   * @throws IOException
   * @see <a
  ...
  /**
   * Authenticated resource which lets you generate an order associated with a button. After
   * generating an order, you can send Bitcoin to the address associated with the order to complete
   * the order. The status of this newly created order will be ‘new’.
   *
   ...
   * and subscriptions you’ve created with merchants). This is a paged resource and will return the
   ...
   *     href="https://coinbase.com/api/doc/1.0/recurring_payments/index.html">coinbase.com/api/doc/1.0/recurring_payments/index.html</a>
   */
  public CoinbaseRecurringPayments getCoinbaseRecurringPayments(Integer page, final Integer limit)
      throws IOException {
  ...
  /**
   * Authenticated resource that lets you (as a merchant) list all the subscriptions customers have
   * made with you. This call returns {@code CoinbaseSubscription} objects where you are the
   * merchant. This is a paged resource and will return the first page by default.
   *
   * @return
   * @throws IOException
   * @see <a
   ...
   *     href="https://coinbase.com/api/doc/1.0/subscribers/index.html">coinbase.com/api/doc/1.0/subscribers/index.html</a>
   */
  public CoinbaseSubscriptions getCoinbaseSubscriptions(Integer page, final Integer limit)
      throws IOException {

    final CoinbaseSubscriptions subscriptions =
        coinbase.getsSubscriptions(
            page,
            limit,
            exchange.getExchangeSpecification().getApiKey(),
            signatureCreator,
  ...
  /**
   * Authenticated resource that lets you (as a merchant) show an individual subscription than a
   * customer has created with you. This call returns a {@code CoinbaseSubscription} object where
  ...
  public CoinbaseSubscription getCoinbaseSubscription(String subscriptionId) throws IOException {
    ...
    final CoinbaseSubscription subscription =
        coinbase.getsSubscription(
            subscriptionId,
            exchange.getExchangeSpecification().getApiKey(),
            signatureCreator,
            exchange.getNonceFactory());
