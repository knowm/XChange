...
public interface CoinbaseAuthenticated extends Coinbase {
  ...
  /**
   * All API key requests must be signed and contain the following headers. All request bodies
   * should have content type application/json and be valid JSON. The CB-ACCESS-SIGN header is
   * generated by creating a sha256 HMAC using the secret key on the prehash string timestamp +
   * method + requestPath + body (where + represents string concatenation). The timestamp value is
   * the same as the CB-ACCESS-TIMESTAMP header. The body is the request body string or omitted if
   * there is no request body (typically for GET requests). The method should be UPPER CASE. <a
